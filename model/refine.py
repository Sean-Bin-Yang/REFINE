...
class REFINE(nn.Module):
    ...
    def forward(self, node_fea, edge_index, enc_data, dec_data, y2, lambda2=0.5):
        # print(y2.shape) ##32 xxx
        ...
        mask_traj_x, full_traj_x, full_temporal_x, full_user_id_x, full_highway_x, full_temporal_mat_x, full_dis_mat_x, full_padding_mask, mask_idx_x = dec_data
        ...
        full_ce = self.ce(full_highway_x)
        full_fusion = F.relu(self.f_linear1(torch.cat([full_temporal_x, full_ce], dim=-1)))
        full_ue = self.ue(full_user_id_x)
        x_full = spe[full_traj_x] + full_ue + full_fusion
        x_full += self.pe(x_full)

        # print(x_full.shape) ###32 L 128

        ###Full Trajectory Z
        Z = self.enc_dropout(x_full)
        Z = self.encoder(
            x=Z,
        ...
        pred1 = self.task1(x)### Next Segment prediction
        ...
        # print(x.shape)###32 l 128
        pred2 = self.task2(x) ###Reconstruction
        # print(pred2.shape) 32 xx 10542

        ####Z hat
        Z_hat = self.enc_dropout(x)
        Z_hat = self.encoder(
            x=Z_hat,
            mask=full_padding_mask,
            temporal_mat=full_temporal_mat_x,
