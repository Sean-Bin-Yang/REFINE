...
class TrajDataLoader:
    ...
    def _collate_fn(self, batch_data, vocab, pre_len=0):
        ...
        full_user_id, full_mask_traj, full_traj, full_temporal, full_temporal_vec, full_temporal_mat, full_dis_mat, full_highway = zip(*full_data)
        key_user_id, key_traj, key_temporal, key_temporal_vec, key_temporal_mat, key_dis_mat, key_highway = zip(*key_data)
        ...
        full_traj_x = torch.zeros(size=(bz, max_len), dtype=torch.long)
        ...
        for i in range(bz):
            ...
            full_traj_x[i, :full_end]=torch.tensor(full_traj[i], dtype=torch.long)
        ...
        dec_data = [mask_traj_x, full_traj_x, full_temporal_x, full_user_id_x, full_highway_x, full_temporal_mat_x, full_dis_mat_x, full_padding_mask, mask_idx_x]
